...
void Router::rxProcess()
{
	...
	for (int i = 0; i < DIRECTIONS + 2; i++) {
		...
		{
			double this_time = sc_time_stamp().to_double() / GlobalParams::clock_period_ps;
			if (received_flit.src_id == local_id)
			{
				received_flit.prev_push_timetamp = this_time;
			}
			else
			{
				double time_dif = this_time - received_flit.prev_push_timetamp;
				received_flit.prev_push_timetamp = this_time;
				// LOG << " TIME DIF " << received_flit << "  " << time_dif << endl;
				this->phermoneLevels[i] = (this->phermoneLevels[i] + (1.0 / time_dif) ) / 2; // ? Averaging out for now
				// LOG << " PHLEVEL [" << i << "] " << this->phermoneLevels[i];
			}
		  ...
		  if (flit.flit_type == FLIT_TYPE_HEAD) 
		    {
			  ...
			  for (int i = 0; i < 4; i++)
			  {
			    route_data.phermoneLevels[i] = this->phermoneLevels[i];
			  }
